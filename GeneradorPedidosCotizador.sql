...
FROM CotizacionMasterPolaris     
 ...
 BEGIN TRANSACTION    
    INSERT Venta              
      ...
      VALUES (@Sucursal, @Sucursal,'BAVAR', 'Cotizaci¢n Repuestos', @FechaEmision, @FechaEmision,  GETDATE(), 'Quetzales', 1,  ISNULL(@Almacen,'141S'), @Cliente,             
...
DECLARE crExtraeDatos CURSOR LOCAL    
 ...
 SELECT  @ListaPreciosEsp = ListaPreciosEsp FROM VENTA WHERE ID = @IDVenta     
  ...
  SELECT  GETDATE()   , 105      , @Sucursal     , CASE WHEN @EsServicio = 1 THEN 106 ELSE 105 END, @OrigenTipo, @Origen, @OrigenID, @Empresa,       
          @Usuario, 'SINAFECTAR', @GenerarMov, @GenerarMovID, @FechaE?mision, 1 , @Almacen, @Concepto, 'Quetzales', 1, 'Polaris '+ RTRIM(@Origen)+' '+RTRIM(@OrigenID),       
...
-- Finaliza Crear Requisicion Compras */   
 ...
 -- set @Refe=CONCAT('Cotizacion Polaris ',@ID)  
    ...
    exec spVentaRepCteD @Estacion, @Empresa,@Cliente,@Refe  
     IF @Generado = 0    
   SELECT 'Se Gener¢ Cotizaci¢n Repuestos ' + @MovID -- + '<BR>Se Gener¢ Requisici¢n ' + @MovIDCompras    
    
   ELSE     
     SELECT 'Se Gener¢ Cotizaci¢n Repuestos ' + @MovID  /*+ '<BR>Se Gener¢ Requisici¢n ' + @MovIDCompras +'<BR>'+ CAST(@Generado AS VARCHAR(5))*/+ ' Articulo(s) Creado(s) Autom ticamente'    
     END    
